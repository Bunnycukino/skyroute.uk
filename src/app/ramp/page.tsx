  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 1000, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '20px', overflowY: 'auto' }}>
      <div style={{ background: '#fff', borderRadius: 12, maxWidth: 820, width: '100%', overflow: 'hidden', boxShadow: '0 25px 60px rgba(0,0,0,0.3)' }}>
        <div style={{ background: 'linear-gradient(135deg, #1e3a8a, #2563eb)', padding: '16px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
...
export default function RampInputPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setSuccess({ c209: data.c209, c208: data.c208 || '', bar: payload.container_code, flight: payload.flight_number, pieces: payload.pieces, signature: payload.signature, notes: payload.notes, date: new Date(payload.date_received).toLocaleDateString('en-GB'), photo: photoDataUrl || undefined });
      ...
      <main className=\"main-content\" style={{ flex: 1, padding: 32 }}>
        ...
        {success && (
          <div style={{ background: '#10b981', borderRadius: 12, padding: '20px 24px', marginBottom: 24, color: '#fff' }}>
            <div style={{ fontWeight: 700, fontSize: 18 }}>✅ Registration Successful! C209: {success.c209}</div>
            <div style={{ fontSize: 14, marginTop: 4, opacity: 0.9 }}>{success.bar} • {success.flight} • {success.pieces} pcs • {success.date}</div>
          ...
          <div className=\"info-panel\" style={{ background: '#fff', borderRadius: 12, border: '1px solid #e5e7eb', overflow: 'hidden', alignSelf: 'flex-start' }}>
            ...
            <div style={{ padding: 20 }}>
              {[
                ...
                'Entry will be added to expiry tracking (48h)',
                'You can add a confirmation photo of receipt',
                'After saving, you can print the IN BOND Control Sheet',
              ].map(info => (
